...
from app.workroom.models import Issue, Position, Workroom, WorkroomPosition
...
class IssueSerializerTest(TestCase):
    ...
    def test_partial_update_allows_missing_due_date(self):  # due_date 없이 partial update 가능
        issue = Issue.objects.create(
            workroom=self.workroom,
            user=self.user,
            title="T",
            status="pending",
            content="C",
            due_date=self.workroom.start_date,
        )
        serializer = IssueSerializer(issue, data={"title": "New"}, partial=True, context={"workroom": self.workroom})
        self.assertTrue(serializer.is_valid())
...
class WorkroomPositionSerializerTest(TestCase):
    def setUp(self):
        ...
        self.wp = WorkroomPosition.objects.create(
            workroom=self.workroom, position=self.position, count=5, current_count=1
        )
